def call() {
    pipeline {
        ...
        parameters {
            string(name: 'TARGET_URL', defaultValue: 'http://www.webscantest.com', description: 'Enter the target URL for scanning')
        }
        environment {
                ...
                steps {
                    container ('zap') {
                        script {
                            ...
                            sh "curl -s 'http://zap.devops-tools.svc.cluster.local:8090/JSON/reports/action/generate/?title=ZAP%20Security%20Report&template=modern&reportDir=/zap-reports/&reportFileName=modern-report.html'"
                            sh "cp /zap-reports/modern-report.html ."
